...
app.post('/run-code', async (req, res) => {
    ...
    // Убираем лишние пробелы/переносы в конце, но НЕ трогаем начало,
    // чтобы не сбивать нумерацию в самом коде:
    const codeForPrompt = code.trimEnd();
  
    const messages = [
        {
          role: "user",
          content: `Ниже приведён код, который начинается с первой строки сразу после тройных бэктиков. Каждая строка, включая пустые, должна учитываться. Не добавляй и не удаляй строки.
      
      Требования:
      1) Игнорируй однострочные комментарии, начинающиеся с "#".
      2) Проверь код только на синтаксические ошибки. Семантические, логические и другие виды ошибок игнорируй.
      3) Если найдена синтаксическая ошибка, выведи ровно одну короткую строку для каждой строки с ошибкой в формате: "Линия #: краткое описание ошибки". Сообщение должно быть дружелюбным, точным и содержать только необходимую информацию (например, "вы забыли двоеточие после условия if").
      4) Если код не содержит синтаксических ошибок, выведи только результат выполнения кода (как компилятор), без дополнительных сообщений.
      5) Если код запрашивает ввод, сгенерируй случайное значение.
      6) Нумерация строк начинается с первой строки, которая идёт сразу после тройных бэктиков, причём пустые строки тоже считаются.
    ...
    try {
      // Пример: вычитаем 5 iCoins
      const { user, apiResponse } = await callAiApiAndDeductIcoins(req, messages, 5, {
        temperature: 0.1,
        max_tokens: 150,
        top_p: 0.1
      });
      ...
      // Возвращаем обновлённый баланс, чтобы фронт мог его отобразить
...
app.post('/correct-code', async (req, res) => {
    ...
    try {
        ...
        const { user, apiResponse } = await callAiApiAndDeductIcoins(req, messages, 30, {
            temperature: 0.1,
            max_tokens: 500,
            top_p: 0.5
        });
        ...
        if (correctedCode) {
            ...
            return res.json({ success: true, correctedCode, icoins: user.icoins });
        } else {
